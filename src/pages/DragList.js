...
export default class DragList extends Component {
  ...
  getHalls(){
    GetHalls().then((res) => {
      if(res){
        ...
        this.setState({
          halls: hallsArray,
          lowerHalls: hallsArray.filter(hall => hall.unit.replace(/\s+/g, '').toLowerCase() === 'kilobytes' || hall.unit.replace(/\s+/g, '').toLowerCase() === 'megabytes'),
          upperHalls: hallsArray.filter(hall => hall.unit.replace(/\s+/g, '').toLowerCase() === 'gigabytes' || hall.unit.replace(/\s+/g, '').toLowerCase() === 'terabytes'),
        })
      } else {
        this.setState({
          halls: [],
  ...
  async unCallAll(){
    ...
    for (let i = 0; i < this.state.halls.length; i++) {
      ...
      const response = await CallHall(this.state.halls[i].id, true, this.state.halls[i].order)
    }
  }
  async reOrderAll(){
    var i = 0
    // console.log(this.state.halls[0])
    for (let i = 0; i < this.state.halls.length; i++) {
      const response = await Reorder(this.state.halls[i].id, this.state.halls[i].order, i)
    }
  }
    ...
    return (
      <div>
                    ...
                    {(provided, snapshot) => (
                      <div
                        ...
                        <div style={{
                          ...
                          <img src={'https://icon-library.net/images/3-line-menu-icon/3-line-menu-icon-3.jpg'} alt="Logo"
                            style={{
                              margin: 5
                            }}
                            className={'App-drag-img'}
                            />
                        </div>
                        <Spinner animation="border" variant="dark"/>
                        <AdminHall hallFunc={this.getHalls} data={hall} idx={index}/>
                      </div>
                    )}
                  </Draggable>
                ))}
        ...
        <Form
          ...
          buttonText="Add Hall"
          onSubmit={(data) => NewHall(data.formData, this.state.halls.length).then(this.getHalls)}
          />
        <Form
          schema={alertFormParams}
          className={'form form-wide'}
          onSubmit={(data) => NewAlert(data.formData.type).then(this.getHalls)}
          />
        <div style={{
            alignItems: 'center',
            display: 'flex',
          ...
          {(this.state.alert.length > 0) ? <Alert alert={this.state.alert} hallFunc={this.getHalls}/> : <div/>}
        </div>
        <button
        ...
        <button
          onClick={() => NewAlert('MED CALL IN THE HALL').then(this.getHalls)}
          style={{
            backgroundColor: GLOBALS.orange,
            color: 'white',
            borderColor: 'transparent'
